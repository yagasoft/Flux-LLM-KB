...
| V3 Scale And Evaluation | Prove Flux improves retrieval and memory quality before trusting more automation. | in progress | 59% | Code-aware corpus indexing, historical backfill, retrieval benchmarks, automated memory governance, optional ParadeDB/BM25, and local-only librarian workers. Code-aware foundations, expanded parser/relationship coverage, metadata-only retrieval benchmark calibration, governance-shadow evaluation, approved Codex backfill ingestion, and privacy-safe code retrieval feedback now exist; broader governance automation remains planned. | Use live repository feedback and benchmark failures to drive the remaining parser/ranking work, librarian shadow workers, reversible governance automation, optional backend decisions, and broader historical backfill discovery. |
...
| Retrieval benchmarks | P0 | Measure whether Flux search is actually finding the right things. | Retrieval benchmark suite. | in progress | 82% | The deterministic synthetic suite now seeds temporary public-safe retrieval cases, calls search/explain/brief paths, records metadata-only history in `retrieval_benchmark_runs`, and exposes REST, CLI, MCP, and dashboard surfaces. Metrics include top-1 accuracy, precision@3, recall@5, MRR, nDCG@5, brief recall, brief dilution, scope pass counts, suppression pass counts, elapsed time, metric deltas, confidence-band summaries, semantic-threshold advisory candidates, and sanitized case failures for mail/file filters, current-only, lifecycle-deprioritized, semantic duplicate/guardrail, code-symbol misses, callers, tests, routes, generated-file handling, config/migration lookup, fallback recovery, and cross-root disambiguation. The `governance-shadow` suite adds metadata-only stale, duplicate, contradicted, low-confidence, protected/current, and false-positive guardrail cases plus proposal counts, categories, guardrail pass/fail counts, precision-style summaries, and `settings_mutated: false`. | Use benchmark/live feedback for ranking/parser tuning and evaluated librarian-worker automation. Do not mutate settings, thresholds, ranking, retention policy, semantic clusters, or lifecycle state automatically. | Use benchmark/live feedback for ranking/parser tuning and evaluated librarian-worker automation. Do not mutate settings, thresholds, ranking, retention policy, semantic clusters, or lifecycle state automatically. |
...
| Librarian workers | P2 | Let Flux suggest cleanup candidates so memory does not become stale or cluttered. | Automation-first local librarian workers for derived-memory cleanup, consolidation, and linting. | in progress | 15% | A read-only governance-shadow proposal evaluator now uses retention-quality, lifecycle, duplicate, stale, low-confidence, contradiction, and canonical evidence to produce metadata-only proposal candidates for benchmark evaluation. No worker scheduling or automatic apply path exists. | Add explicit shadow-mode workers, threshold calibration, recovery links, and optional local-model rationale only after governance-shadow evidence is stable. | Add explicit shadow-mode workers, threshold calibration, recovery links, and optional local-model rationale only after governance-shadow evidence is stable. |
| Automated memory governance | P2 | Allow safe routine cleanup only after tests prove it will not damage useful memory. | Policy-gated automation for routine memory lifecycle optimization. | planned | 20% | Retention quality reporting, lifecycle primitives, approved-ingestion gates, and governance-shadow evaluation exist, but automatic mutation is not implemented. | Define evaluation thresholds, reversible apply/recover flows, duplicate suppression, retrieval deprioritization, stale tagging, canonical cluster presentation, and operator digest behavior before any automatic lifecycle mutation. | Define evaluation thresholds, reversible apply/recover flows, duplicate suppression, retrieval deprioritization, stale tagging, canonical cluster presentation, and operator digest behavior before any automatic lifecycle mutation. |
| Local consolidation and escalation | P2 | Combine obvious duplicates locally and ask a person only for risky decisions. | Local-only consolidation with rare human escalation. | planned | 5% | Canonical semantic/procedural consolidation and escalation policy are not implemented. | Use local model-assisted consolidation only when evidence is high-confidence and provenance is preserved; escalate only for hard deletion, privacy/security findings, high-authority contradictions, destructive policy changes, protected memories, low-confidence high-impact decisions, or failed evaluation thresholds. | Use local model-assisted consolidation only when evidence is high-confidence and provenance is preserved; escalate only for hard deletion, privacy/security findings, high-authority contradictions, destructive policy changes, protected memories, low-confidence high-impact decisions, or failed evaluation thresholds. |
| Operator digests | P2 | Summarize what automation did or wants to do so the operator stays in control. | Periodic reporting for automated memory governance. | planned | 0% | Review UI exists, but automation digests are not implemented. | Provide periodic digests and recovery/audit views for operator awareness instead of per-item approval. | Provide periodic digests and recovery/audit views for operator awareness instead of per-item approval. |
...
6. Priority: P2. Plain-English purpose: improve memory quality with reversible evaluated
   automation. Bundle: Evaluated Memory Governance Automation. Progress: 20%.
   Model-actionable item: build on the completed review/evaluation gate by
   adding explicit librarian-worker shadow mode, benchmarked thresholds,
   ...
   model routing with rule-based fallback.
   Remaining Work: define evaluation thresholds, worker cadence, reversible
   apply/recover flows, operator digests, protected-memory rules, and
