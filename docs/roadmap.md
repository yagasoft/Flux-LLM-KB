...
| V2 Review And Visualization | Give the operator a dashboard to inspect, correct, and trust what Flux remembers. | in progress | 88% | React dashboard is the unified operational UI; graph browsing, claim lifecycle review, capture-review visibility, approval/rejection decisions, approved-backfill ingestion, audit-visible rationales, retention policy tuning, memory quality reporting, operator evidence gates, code feedback, filtered operational diagnostics, and row-level diagnostic remediation actions exist. | Complete deeper exception-oriented review workflows, live validation of recovery actions, and higher-value recovery pages while keeping automatic lifecycle mutation out of the manual review slice. |
...
| Piece | Priority | Plain-English Purpose | Roadmap Intent | Status | Progress % | Current Evidence / Remaining Gap | Remaining Work | Queued Next |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Unified operations dashboard | P1 | Put health, review, search, and controls in one place so operators do not need raw logs. | Unified dashboard for health, monitoring, watcher status, crawler stats, search, graph browsing, stale claims, contradictions, capture review, runtime settings, and mail ingestion status. | in progress | 88% | React/Vite dashboard served by FastAPI is the unified operations UI for health, corpus monitoring, runtime settings, mail capture, worker state, Outlook COM host status, claim lifecycle review, selected-entity graph browsing, retention tuning, memory quality reporting, capture-review queue decisions, approved-ingestion status/counters, governance-shadow benchmark evidence, and recent decision/ingestion audit visibility. | Keep retention review exception-oriented for rare escalations, policy tuning, audits, and recovery once automatic retention workers exist. | Keep retention review exception-oriented for rare escalations, policy tuning, audits, and recovery once automatic retention workers exist. |
...
| Observability and benchmarks | P0 | Give operators before/after evidence instead of guessing which setting or update helped. | Dashboard panels and benchmark fixtures for throughput, latency, cache hits, model warm/cold state, CPU/GPU mode, blocked dependencies, slow files, and p50/p95 indexing times. | in progress | 88% | Dashboard Health and Jobs show acceleration capabilities, selected watcher backend, cache root, worker-family queue counts, cap/backpressure status, p95 duration where available, OCR/ASR/container/parser/embedding telemetry, manifest skip counters, slow-job diagnostics, durable benchmark history, reliability/code/operational diagnostic panels, and all-root reliability matrix. Benchmark run modes cover `scan`, `soak`, `watcher`, `model`, and `all`; scenario runners cover `standard`, `reliability`, `host_cloud`, `cache_readiness`, and `tuning`, return `diagnostics[]`, keep `settings_mutated: false`, and expose manual `recommendations.candidates[]`. Stored benchmark records are metadata only and exclude raw text, private watched roots, mail contents, credentials, and embeddings. | Compare benchmark history across deployment updates, tune defaults from observed hardware evidence, and keep automatic settings mutation out of scope. | Use all-root reliability, code diagnostics, and operational diagnostics as the operator evidence layer; do not add automatic settings mutation. |
