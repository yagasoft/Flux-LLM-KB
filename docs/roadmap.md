...
| Drill-down diagnostics | P1 | Let operators investigate why Flux behaved a certain way instead of guessing. | Dashboard drill-down views for retrieval explanations, watcher events, and worker history. | in progress | 80% | Dashboard has operational panels plus read-only diagnostics for retrieval explain traces, watcher events, worker heartbeat/history, slow jobs, blocked dependencies, mail sync runs, and mail post-process state. Deeper row-level investigation and richer remediation workflows remain uneven. | Add row-level detail pages, filtering, retention/recovery actions, and remediation workflows after the read-only diagnostic evidence is validated. | Validate the new read-only panels, then add row-level investigation and recovery workflows where operators actually need them. |
...
| File-type coverage matrix | P1 | Make clear what Flux can read now, what it only tracks as metadata, and what still needs work. | Broad file-type coverage through explicit support tiers and a dedicated coverage matrix. | in progress | 76% | Public `file-type-coverage.md` and broad roadmap targets exist; structured Draw.io, modern VSDX extraction, local business document expansion for Office variants, OpenDocument files, legacy Excel/PowerPoint adapters, EPUB/FB2 extraction, Calibre `ebook-convert` eBook fallback, comic archive container handling, bounded recursive archive/container extraction, embedded media sidecar transcripts, cache-backed local OCR for image/image-only PDF jobs, and cache-backed local ASR for audio/video jobs exist; many advanced formats remain metadata-first or optional-tool dependent. | Continue extractor expansion, prioritizing richer vision, embedded-media diagnostics, and remaining specialized local-tool stages. | Continue extractor expansion, prioritizing richer vision, embedded-media diagnostics, and remaining specialized local-tool stages. |
| Text/code/data extraction | P1 | Read common plain-text, code, and data files quickly without expensive tools. | Inline text extraction and local parser extraction for text, code, developer artifacts, structured data, and common exports. | in progress | 83% | Small text-like files, code parser paths, and sample-first large CSV/TSV/JSON/JSONL/workbook indexing are supported. Optional full structured backfill, converted legacy/ODS sample-first handling, and wider local parsers remain planned. | Extend sample-first behavior to converted legacy/ODS spreadsheet flows where reliable, add optional full structured backfill, and broaden local parser coverage. | Extend structured sample-first coverage and optional backfill only where local parsing is reliable and public-safe tests can cover it. |
| Documents, spreadsheets, and presentations | P1 | Extract useful text from normal business files people already have. | Local extraction for PDF, Office, OpenDocument, ebooks, scans, speaker notes, tables, and embedded media metadata. | in progress | 76% | Common PDF/Office extraction exists; Office macro/template variants, OpenDocument text/spreadsheet/presentation files, legacy Excel/PowerPoint local adapters, bounded OCR fallback for image-only PDFs, EPUB/FB2 local publication parsing, Calibre `ebook-convert` fallback for MOBI/AZW/LIT, comic archive container extraction, and embedded document parsing from bounded containers now have extraction paths. Broader document families still need staged local-tool handling. | Prioritize remaining local-tool support with metadata-first fallback. | Prioritize remaining local-tool support with metadata-first fallback. |
...
| Error diagnostics and operator UX | P1 | Turn failures into actionable messages instead of vague errors. | Standard API error envelopes and dashboard alerts with code, severity, component, target metadata, retryability, user action, technical detail, and links. | in progress | 83% | Structured API error envelopes, dashboard actionable diagnostics, expandable details, copyable JSON, navigation targets, and read-only diagnostic panels for retrieval, watcher, workers, slow/blocked jobs, and mail evidence exist. Deeper remediation workflows remain planned. | Add operator remediation flows, row-level drill-down filters, and recovery links once the read-only diagnostic panels identify recurring failures. | Use the diagnostics panels to identify the highest-value remediation actions before adding mutation flows. |
