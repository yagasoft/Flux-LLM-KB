...
| V2.5 Autonomous Corpus Expansion | Let Flux safely notice and index useful local files without constant manual imports. | in progress | 85% | Watch roots, host agent, reconciliation, worker processing, duplicate/version suppression, broad file-type roadmap, structured diagram extraction, business document extractor expansion, converted legacy/OpenDocument spreadsheet sample-first profiling, and bounded recursive archive/container extraction exist; deeper media parsing remains planned. | Finish remaining file-type stages, richer media/vector diagnostics, optional-tool validation, optional full structured backfill, and live sync-client proof. |
...
| File-type coverage matrix | P1 | Make clear what Flux can read now, what it only tracks as metadata, and what still needs work. | Broad file-type coverage through explicit support tiers and a dedicated coverage matrix. | in progress | 78% | Public `file-type-coverage.md` and broad roadmap targets exist; structured Draw.io, modern VSDX extraction, local business document expansion for Office variants, OpenDocument files, legacy Excel/PowerPoint adapters, converted large legacy/OpenDocument spreadsheet sample-first profiles, EPUB/FB2 extraction, Calibre `ebook-convert` eBook fallback, comic archive container handling, bounded recursive archive/container extraction, embedded media sidecar transcripts, cache-backed local OCR for image/image-only PDF jobs, and cache-backed local ASR for audio/video jobs exist; many advanced formats remain metadata-first or optional-tool dependent. | Continue extractor expansion, prioritizing richer vision, embedded-media diagnostics, optional-tool validation, and remaining specialized local-tool stages. | Continue extractor expansion, prioritizing richer vision, embedded-media diagnostics, optional-tool validation, and remaining specialized local-tool stages. |
| Text/code/data extraction | P1 | Read common plain-text, code, and data files quickly without expensive tools. | Inline text extraction and local parser extraction for text, code, developer artifacts, structured data, and common exports. | in progress | 85% | Small text-like files, code parser paths, sample-first large CSV/TSV/JSON/JSONL/workbook indexing, and converted legacy/OpenDocument spreadsheet sample-first profiling are supported. Optional full structured backfill and wider local parsers remain planned. | Add optional full structured backfill only after sample-first metadata proves useful, and broaden local parser coverage where public-safe tests can cover it. | Extend structured-file backfill and parser coverage only where local parsing is reliable and diagnostic evidence shows value. |
...
| Logical related evidence | P1 | Show attachments, archive contents, and sidecars together so evidence stays understandable. | Archive members, mail attachments, embedded objects, and sidecars should appear under parent logical results. | in progress | 76% | Mail spool siblings, archive/container children, and known child assets are grouped as related evidence. Broader embedded media extraction still needs validation. | Extend related-evidence grouping as embedded media extraction lands. | Extend related-evidence grouping as embedded media extraction lands. |
