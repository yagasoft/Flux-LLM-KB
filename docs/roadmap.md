...
| Piece | Plain-English Purpose | Roadmap Intent | Status | Current Evidence / Remaining Gap | Queued Next |
| --- | --- | --- | --- | --- | --- |
| Team/shared vault mode | Let multiple people share knowledge only after personal safety rules are proven. | Team/shared vault mode. | planned | Personal/local-first mode remains the focus. | Define trust, visibility, and audit boundaries after single-user governance matures. |
| Sync and export policies | Decide what can move between machines or users and under what rules. | Sync and export policies. | planned | Export exists for local audit; multi-user sync/export governance is not started. | Design policy model after shared vault requirements are clear. |
