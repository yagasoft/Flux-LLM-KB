...
The design is grounded in the native document publication and retrieval contracts. Repository cleanup uses main `270c94545399790c33aa766e39b5925cd0a13679` as its baseline. Recheck the current implementation before beginning this planned work; the architecture and scoped delivery evidence describe the completed OCR/Visio boundaries.
...
Before implementation, inspect current main, [architecture](../architecture.md) and the [OCR delivery evidence](../operations/2026-09-20-english-ocr-live-delivery.md). Check `DocumentPublications`, `DocumentMetadataJson`, normalisation offsets, deletion and search composition. If behaviour differs materially from this design, amend and review the affected contract before coding.
...
Plain text always receives verified canonical spans. Add canonical line ranges only when computed from the retained normalised text; do not call those original-file line numbers. Office/Visio/table/shape/section locations appear only when the accepted metadata supports the exact passage. Missing retained location metadata yields `location-unavailable`, with valid text-span evidence. Invalid metadata yields a named provenance warning and span-only citation, never a guessed page. Retain provider uncertainty flags where supplied; do not infer that missing flags mean high confidence.
