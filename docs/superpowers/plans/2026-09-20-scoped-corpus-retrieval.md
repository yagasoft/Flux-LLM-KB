...
Date: 2026-09-20
Status: planning complete only; reconcile with the merged OCR branch before implementation.
...
- [ ] Merge completed main into this task branch only after the OCR task has finished its authorised closeout. Preserve these planning documents and resolve only their conflicts; record the resulting commit in the spec.
...
- [ ] Read current `AGENTS.md`, `global.json`, native test instructions and the installed native plugin source manifest. Locate its operation ledger with `rg -n 'knowledge.search|corpus.query' src tests scripts plugins`; update the actual owner files, not an installed plugin copy.
...
- `src/FluxKnowledge.Application/Search/HybridSearchService.cs` and `ReciprocalRankFusion.cs`: reuse ranking infrastructure without changing legacy result contracts.
...
- `src/FluxKnowledge.Infrastructure.SqlServer/Search/SqlFullTextSearch.cs`: share candidate logic where the existing contract permits; preserve legacy callers.
...
- [ ] Write failing citation tests for FormKC length changes already handled by incoming normalisation, CRLF, surrogate pairs, zero-length/invalid spans, multiple pages, native and OCR blocks, absent metadata and corrupt metadata. Assert spans against retained substrings, not copied expected offsets from implementation.
