...
3. Broad `kb.brief`, `kb.search`, and `kb.explain` exclude code by default. If you want code from a broad lookup, pass `filters={"file_kinds":["code"]}` with the query; free-text code-looking words are not enough.
