...
| `mcp__flux_llm_kb.kb_search` | Mid-turn targeted discovery when local files, the prompt, or current tool output do not answer the question. | Use normal kb.brief/search for broad context. Use `scope_mode="workspace_boosted"` for prior decisions, unresolved project context, cross-workspace patterns, general indexed documents, previous fixes, or user-referenced history. |
| `mcp__flux_llm_kb.kb_code_search` | Code-specific lookup for symbols, definitions, paths, parser metadata, or implementation evidence. | Prefer `kb.code_search` or `file_kind=code` filters for code-specific lookup instead of making broad search code-first. |
...
3. If broad `kb.brief` or `kb.search` returns code-heavy results for a non-code prompt, rerun with non-code file kinds such as `file_kind=text`, `file_kind=document`, or `file_kind=image` before injecting context.
