...
try {
  ...
  if ($null -ne $docker) {
    docker compose stop api worker search-index-worker mail-worker automation-worker governance-worker runtime-control-worker callback-worker event-scheduler outbox-relay | Out-Host
  }
