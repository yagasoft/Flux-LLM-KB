...
try {
  ...
  if ($null -ne $docker) {
    docker compose up -d --build postgres rabbitmq api outbox-relay event-scheduler worker search-index-worker mail-worker automation-worker governance-worker runtime-control-worker callback-worker
