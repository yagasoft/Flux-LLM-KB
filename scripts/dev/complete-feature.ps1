...
try {
    ...
    if (-not $SkipDeploy) {
        ...
        $deployDockerBaseMode = "$env:FLUX_KB_DEPLOY_DOCKER_BASE_MODE".Trim()
        if ($deployDockerBaseMode) {
            if ($deployDockerBaseMode -notin @("auto", "local", "python")) {
                throw "FLUX_KB_DEPLOY_DOCKER_BASE_MODE must be one of: auto, local, python."
            }
            $deployCommand = "$deployCommand -DockerBaseMode $deployDockerBaseMode"
        }
