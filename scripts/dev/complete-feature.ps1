...
function Record-NativeGoLiveFailure {
    ...
    if ($Exception.Message -cmatch "\A(?:Native go-live failed with safe reason code '(go-live-(?:acknowledgement-required|cancelled-before-admission|lease-unavailable|closeout-capability-(?:unrecognised|expired|binding-mismatch|consumed))|clean-slate-(?:incomplete|admission-failed)|native-go-live-bootstrap-(?:reset|install|probe)-(?:connection|sni-load|script-parse|sql-batch-[1-9][0-9]*)-failed)'\.|(native-go-live-bridge-(?:composition|invocation|discovery|call|result)-failed))\z") {
