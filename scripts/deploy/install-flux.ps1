...
function Set-FluxProductionRuntimeSettings {
    ...
    try {
        ...
        if ($GpuEnabled) {
            ...
            Invoke-FluxSettingsCommand -VenvPython $VenvPython -Arguments @("settings", "set", "gpu.scheduler.allocator_trim_enabled", "true", "--confirm")
            Invoke-FluxSettingsCommand -VenvPython $VenvPython -Arguments @("settings", "set", "gpu.scheduler.idle_unload_enabled", "true", "--confirm")
            Invoke-FluxSettingsCommand -VenvPython $VenvPython -Arguments @("settings", "set", "gpu.scheduler.idle_unload_seconds", "120", "--confirm")
            Invoke-FluxSettingsCommand -VenvPython $VenvPython -Arguments @("settings", "set", "worker.gpu_busy_retry_block_after_seconds", "3600", "--confirm")
...
function Assert-FluxWheelhouseCacheReady {
    ...
    if ((Get-FluxWheelhouseFileCount -WheelhousePath $WheelhousePath) -eq 0) {
        throw "Durable pip wheelhouse is empty at $WheelhousePath. Seed it explicitly with: .\scripts\deploy\install-flux.ps1 -PipOffline:`$false"
    }
