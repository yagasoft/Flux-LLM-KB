...
def test_code_heavy_benchmark_fixture_covers_code_indexing_paths(tmp_path):
    ...
    service_module._write_benchmark_fixture(root, "code-heavy", 11)
    ...
    assert any(reference["relationship_kind"] == "route" and reference["target"] == "/api/orders/:orderId" for reference in routes.metadata["code"]["references"])
...
def test_code_heavy_benchmark_runs_through_public_service_surface(monkeypatch):
    ...
    result = KnowledgeService().run_benchmark(fixture="code-heavy", files=11, mode="scan")

    assert result["fixture"] == "code-heavy"
    assert result["runs"][0]["fixture"] == "code-heavy"
    assert recorded[0]["fixture"] == "code-heavy"
    assert recorded[0]["file_count"] == 11
    assert recorded[0]["worker_family_breakdown"]["text"]["files"] == 11
