...
def test_complete_feature_deploys_with_pip_offline_only():
    complete_feature = _dev_script("complete-feature.ps1")

    assert "[switch]$AllowPipDownloads" not in complete_feature
    assert "[switch]$RefreshPipDependencies" not in complete_feature
    assert "AllowPipDownloads" not in complete_feature
    assert "RefreshPipDependencies" not in complete_feature
    assert "PipOffline:$false" not in complete_feature
    assert "$pipOffline" not in complete_feature
    assert "$deployPipOfflineValue" not in complete_feature
    assert "-PipOffline:`$true" in complete_feature
    assert "Closeout runs pip offline only." in complete_feature
