...
public sealed class Phase3BCorpusAndEventsBrowserTests
{
    ...
    public async Task Synthetic_SQL_records_are_browsable_in_Corpus_and_durable_Events()
    {
        ...
        try
        {
            ...
            await page.GetByText("File name", new PageGetByTextOptions { Exact = true }).WaitForAsync();
            await page.GetByRole(AriaRole.Link, new() { Name = "browser.txt", Exact = true }).WaitForAsync();
            ...
            await page.GetByRole(AriaRole.Link, new() { Name = "browser.txt", Exact = true }).ClickAsync();
            ...
            await page.GotoAsync(new Uri(host.BaseAddress, "corpus").ToString(), new PageGotoOptions { WaitUntil = WaitUntilState.NetworkIdle });
            await page.GetByRole(AriaRole.Link, new() { Name = "deferred.pdf", Exact = true }).ClickAsync();
            ...
            await page.GotoAsync(new Uri(host.BaseAddress, $"sources/{root:D}").ToString(), new PageGotoOptions { WaitUntil = WaitUntilState.NetworkIdle });
            await page.GetByRole(AriaRole.Heading, new() { Name = "Files", Exact = true }).WaitForAsync();
            await page.GetByRole(AriaRole.Cell, new() { Name = "browser.txt", Exact = true }).WaitForAsync();
            await page.GetByRole(AriaRole.Cell, new() { Name = "deferred.pdf", Exact = true }).WaitForAsync();
